...
const checkOut = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const subTotal = productDetails.reduce((acc, item) => acc + item.price, 0);

    const tax = 0;
    const grandTotal = subTotal + tax;

    //clear cart
    axios.get(`${CART_SERVICE_URL}/cart/clear`, {
      headers: {
        "x-cart-session-id": parseBody.data.cartSessionId,
      },
    });
    ...
    //Send Email
    axios.post(`${EMAIL_SERVICE_URL}/email/send`, {
      recipient: parseBody.data.userEmail,
      subject: "Order Confirmation",
      body: `Your order has been placed successfully. Order ID: ${order.userId}, Your Order Total: ${subTotal}`,
      source: "checkout",
    });
